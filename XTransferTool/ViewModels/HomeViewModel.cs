...
public partial class HomeViewModel : ViewModelBase
{
    ...
    [RelayCommand]
    private async Task SendDemoMessage()
    {
        // V1 demo: send a message to ourselves via local gRPC host.
        var (demoFromId, demoDisplay) = SenderIdentity();
        var request = new SendMessageRequest
        {
            RequestId = Guid.NewGuid().ToString(),
            FromId = demoFromId,
            ToPeerId = demoFromId,
            Text = "这是一条演示留言：请帮忙投到主屏",
            TransferId = "",
            TsMs = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()
        };
        request.FileNames.Add("演示文件.pptx");

        await _messaging.SendAsync("127.0.0.1", 50051, request);

        // Also write to local inbox for immediate UI preview.
        if (_inbox is not null)
        {
            await _inbox.AddRecordAsync(new InboxItemRecord(
                ...
                FromDisplay: demoDisplay,
                TransferId: request.TransferId,
                FileName: request.FileNames.FirstOrDefault() ?? "",
                SavedPath: "",
    ...
    [RelayCommand]
    private async Task CreateDemoTransfer()
    {
        var (demoFromId, demoDisplay) = SenderIdentity();
        var req = new CreateTransferRequest
        {
        ...
        var res = await _fileTransfer.CreateTransferAsync("127.0.0.1", 50051, req);
    ...
    [RelayCommand]
    private async Task UploadDemoChunks()
    {
        // Create transfer then upload random bytes to demonstrate UploadChunks.
        var (demoFromId, demoDisplay) = SenderIdentity();
        var req = new CreateTransferRequest
        {
            RequestId = Guid.NewGuid().ToString(),
        ...
        var res = await _fileTransfer.CreateTransferAsync("127.0.0.1", 50051, req);
        var uploadRes = await _fileTransfer.UploadRandomAsync(
            "127.0.0.1",
            50051,
            res.TransferId,
            res.UploadToken,
        ...
        var completeRes = await _fileTransfer.CompleteTransferAsync("127.0.0.1", 50051, new CompleteTransferRequest
        {
            TransferId = res.TransferId,
            UploadToken = res.UploadToken,
            Items =
            {
                ...
                {
                    ItemId = itemId,
                    SizeBytes = req.Items[0].SizeBytes
                }
            }
        });
        ...
        if (_inbox is not null)
        {
            await _inbox.AddRecordAsync(new InboxItemRecord(
                ...
                TransferId: res.TransferId,
                FileName: req.Items[0].FileName,
                SavedPath: "(see .recv folder under app base dir)",
                Message: $"UploadChunks={uploadRes.Status} accepted={uploadRes.AcceptedBytes}; CompleteTransfer={completeRes.Status}",
                Status: "unread"
            ));
        }
    }
